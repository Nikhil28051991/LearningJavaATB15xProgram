...
public class VerifyCredentialPage {
    ...
    public void verifyCredentialFlow() throws InterruptedException {
        ...
        if (!newWindowOpened) {
            // Verify might have opened in a modal/dialog in the same window
            ...
            cleanupAfterVerification(mainWindow, false);
            return;
        }

        // ── Prefer the verifier-domain tab when Chrome opens multiple tabs (Sign-up vs Verifier) ──
        ...
        System.out.println("Verifier-focused tab URL: " + currentTabUrl);
        System.out.println("Verifier-focused tab title: " + driver.getTitle());
        ...
        if (isAuthRedirect && verifyUrl != null && !verifyUrl.isEmpty() && !verifyUrl.contains("/auth/")) {
            System.out.println("Auth redirect detected. Navigating directly to verification URL: " + verifyUrl);
            ...
            System.out.println("After direct navigation — URL: " + currentTabUrl);
        }
        ...
        // Let verifier UI run ~4–5s after clicking Verify (per requirement), then wait for completion
        System.out.println("Waiting ~4.5s after Verify click for verification to progress...");
        Thread.sleep(4500);
        waitForVerificationComplete();

        // Stay 2 seconds so the user can read the final result
        Thread.sleep(2000);

        // ── Close the verifier tab and return to main window ──
    ...
    private void cleanupAfterVerification(String mainWindow, boolean closeCurrentTab) throws InterruptedException {
        ...
        Thread.sleep(300);
        System.out.println("✅ Back on main window — navigating to Credentials > Issued tab");
        try {
            ...
            System.out.println("Post-verify navigation: " + e.getMessage());
        }
    }
