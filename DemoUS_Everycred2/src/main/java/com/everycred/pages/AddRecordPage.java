...
private void waitForDialogFormContent() {
    ...
    try {
        ...
        System.out.println("Form fields not visible after 12s — proceeding anyway: " + e.getMessage());
    }
}

private void openAddRecordFormPopup() {
    By dialogLocator = By.xpath("//div[contains(@class,'p-dialog')]");
    // If dialog already open, verify form content is rendered before returning
    if (!driver.findElements(dialogLocator).isEmpty()) {
        collapseGetStartedPanelIfPresent();
        dismissOnboardingCompleteIfPresent();
        waitForDialogFormContent();
        return;
    }
    ...
    for (int attempt = 1; attempt <= 4; attempt++) {
        try {
            ...
            collapseGetStartedPanelIfPresent();
            dismissOnboardingCompleteIfPresent();
            // Then wait for actual form fields to render inside the dialog (fixes blank form issue)
            waitForDialogFormContent();
            System.out.println("Add Record form popup opened");
            return;
        } catch (Exception e) {
            lastError = e;
            delay();
    ...
    throw new RuntimeException("Unable to open Add Record form popup", lastError);
}
...
collapseGetStartedPanelIfPresent();
dismissOnboardingCompleteIfPresent(); // dismiss "Onboarding Complete" modal if all 5 steps done
