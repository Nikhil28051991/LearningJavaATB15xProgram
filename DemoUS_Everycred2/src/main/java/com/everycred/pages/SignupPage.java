...
public class SignupPage {
    ...
    private void submitSignUpForm() throws InterruptedException {
        ...
        By verifyingState = By.xpath(
                "//button[contains(normalize-space(),'Verifying Email')] | " +
                "//*[contains(normalize-space(),'Verifying Email')]");

        By captchaError = By.xpath(
                "//*[contains(normalize-space(),'captcha') or contains(normalize-space(),'Captcha') " +
                "   or contains(normalize-space(),'CAPTCHA') or contains(normalize-space(),'robot') " +
                "   or contains(normalize-space(),'Invalid captcha') or contains(normalize-space(),'invalid captcha')]");
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            while (System.currentTimeMillis() < pollDeadline) {
                ...
                // ⚠️ Captcha error — wait 6s, then loop again (re-fill + retry submit)
                ...
                if (!capElems.isEmpty()) {
                    ...
                    System.out.println("Captcha error detected: [" + capText + "] — waiting 6s then retrying...");
                    Thread.sleep(6000);
                    captchaDetected = true;
                    break; // break poll loop → re-fill and retry in next for-loop iteration
                }

                // ❌ Other server error (duplicate email, validation etc.) — stop and go to YOPmail
                ...
                if (!errElems.isEmpty()) {
                    ...
                    System.out.println("Account may already exist. Proceeding to YOPmail.");
                    // Email may have been sent in a previous run — let YOPmail check
                    emailSent = true;
                    break;
                }
        ...
        // Wait for the "Verifying Email..." state to disappear (server finished processing)
        try {
            new WebDriverWait(driver, Duration.ofSeconds(15))
                    .until(ExpectedConditions.invisibilityOfElementLocated(verifyingState));
            System.out.println("Verification processing complete — confirmation email dispatched");
        } catch (TimeoutException e) {
            System.out.println("Loading state still visible — waiting 3 more seconds...");
            Thread.sleep(3000);
        }

        // Brief pause so the success toast is visible before opening YOPmail
        Thread.sleep(2000);
        System.out.println("Sign Up submitted — confirmation email sent to " + EMAIL);
        System.out.println("Opening YOPmail now...");
    }
    ...
    private void refreshYopmailInbox() {
        try {
            driver.switchTo().defaultContent();
            // Re-enter username and click Go to refresh inbox
            ...
            System.out.println("Inbox refresh failed: " + e.getMessage());
        }
    }
